"""
    ...
    2018/09/17
        ...
        tries to search for SMILES structures for compounds that do not have them 
"""
...
def save_search_cache(search_cache):
    """
save_search_cache
    description:
...
"""
    cache_name = os.path.join(os.path.split(__file__)[0], 'search_cache.json')
...
def cid_from_name(sess, name, delay=0.2):
    """
smi_from_cid
    description:
...
def fetch_smi_byname(cursor, session, search_cache, delay=0.2, gen_lipid_smi=True):
    """
fetch_smi_byname
    description:
