...
def main(input_file, calibration_file):
    ...
    for i, row in df_input.iterrows():
        ...
        # Extract m/z,rt-selected EIM
        for rt_value, peak_area in zip(rt_list, areas):
            t, dt_i = rdr.get_filtered_chrom(1, float(mz), 0.025, rt_min=float(rt_value)-0.1, rt_max=float(rt_value)+0.1)  # Adjust the MS function and mass tolerance window
...
if __name__ == "__main__":
    input_file = "2023_07_27_RO1_pooled_feces_30B_100B.xlsx" # Enter path to the processing template containing file names
    calibration_file = "2023_07_27_polyala_mz_ccs.xlsx" # Enter path to the CCS calibrant 
